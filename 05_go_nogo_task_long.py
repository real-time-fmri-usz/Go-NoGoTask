...
trial_list = pd.read_csv(filepath_or_buffer = "trials_long/no_go_catch_run"+run+"-001.par",
delim_whitespace=True,
header = None,
names = ['totalDur', 'trialType', 'duration', 'weirdFactor', 'trialName'])
trial_list = trial_list.replace(np.nan,'Null')
go_type = ''
strength = ''
last_trial_dur = 0
trial_counter = 0
...
for trial in range(len(trial_list)):
		...
		if trial_list['trialName'][trial] == 'Null':
			elapse_time += last_trial_dur
			trial_duration = int(trial_list['duration'][trial] * 60)
			for frames in range(trial_duration):
				win.flip()
			output_file.write(','.join([str(subid),str(session),str(run),str(trial+1),'Rest','Rest','Rest','Rest','Rest','Rest','Rest',str(elapse_time/60),str(trial_list['duration'][trial]),'Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest',str(trial_list['duration'][trial])+'\n']))
			output_file.flush()
			...
			if trial_counter == 36 or trial_counter == 72 or trial_counter == 108:
				for p in range(int(pause_dur[0])):
					pause_text.draw()
					win.flip()
				elapse_time += pause_dur[0]
				pause_dur.pop(0)

			if trial_list['trialName'][trial] == 'weakGo':
				strength = 'weak'
				go_type = 'go'
			elif trial_list['trialName'][trial] == 'strongGo':
				strength = 'strong'
				go_type = 'go'
			elif trial_list['trialName'][trial] == 'weakNoGo':
				strength = 'weak'
				go_type = 'nogo'
			elif trial_list['trialName'][trial] == 'strongNoGo':
				strength = 'strong'
				go_type = 'nogo'
			elif trial_list['trialName'][trial] == 'catch':
				strength = 'catch'
				go_type = 'catch'
