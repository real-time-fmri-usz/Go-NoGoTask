...
dlg.addField('Run:')
dlg.addField('Scanner', choices = ['yes'])
...
dlg.addField('Multiband', choices = ['Singleband', 'Multiband','Outside'])
...
mask_size_ratio = 1.6         #how much proptionally bigger is maskzzzzr
...
session = exp_input[7]
multiband = exp_input[8]

if multiband == 'outside':
    ScreenRes = True
else:
    ScreenRes = False
...
#window
win = visual.Window(size=[800, 600], color=[-1,-1,-1], screen = 0, fullscr = ScreenRes, allowStencil = True)
...
output_file.write('subid,session,run,trial,trial_type,side,response,correct,strength,response_time,cumulative_response_time,fixation_onset,fixation_dur,stim_onset,stim_dur,blank_onset,blank_dur,mask_onset,mask_dur,response_onset,response_dur,go_side,Staircase_stimulus,Presentation Duration, Multiband, fix_onset\n')
output_file.flush()
...
exp_start_time = experiment_clock.getTime()
	...
	for trial in range(len(trial_list_block)):
		if trial_list_block['trialName'][trial] == 'Null':
			...
			for frames in range(trial_duration):
				win.flip()
			output_file.write(','.join([str(subid),str(session),str(run),str(trial+1),'Rest','Rest','Rest','Rest','Rest','Rest','Rest',str(elapse_time/60),str(trial_list_block['duration'][trial]),'Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest',str(trial_list_block['duration'][trial]),'Rest','Rest'+'\n']))
			output_file.flush()
			...
			for s in range(int(fixation_dur)):
				fixation.draw()
				if s == 0:
					fix_onset_time = experiment_clock.getTime() - exp_start_time
				win.flip()
		...
		#response collection
			...
			last_trial_dur = fixation_dur + blank_dur_pre + stim_dur[strength] + blank_dur[strength] + response_dur
			#port.setData(0)
	elapse_time += last_trial_dur
