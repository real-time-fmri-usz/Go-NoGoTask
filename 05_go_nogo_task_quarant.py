...
responses = [90,92,94,96,98,100]
response_dur_list = responses * 24
...
output_file.write('subid,session,run,trial,trial_type,side,response,correct,strength,response_time,cumulative_response_time,fixation_onset,fixation_dur,stim_onset,stim_dur,blank_onset,blank_dur,mask_onset,mask_dur,response_onset,response_dur,go_side,Staircase_stimulus,Presentation Duration,fix_onset\n')
output_file.flush()
	...
	for trial in range(len(trial_list_block)):
		if trial_list_block['trialName'][trial] == 'Null':
			...
			for frames in range(trial_duration):
				win.flip()
			output_file.write(','.join([str(subid),str(session),str(run),str(trial+1),'Rest','Rest','Rest','Rest','Rest','Rest','Rest',str(elapse_time/60),str(trial_list_block['duration'][trial]),'Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest','Rest',str(trial_list_block['duration'][trial]),'Rest'+'\n']))
			output_file.flush()
